...
class FeedForward(nn.Module):
    def __init__(self, dim, expansion=4, activation="mish", dropout=0.1):
        super().__init__()
        hidden_dim = int(dim * expansion)   # Convert to int here!
...
# ReGroup channels to hierarchical groups for heads
...
# Calculate cache for current layer
...
# Modified TransformerBlock to handle qv_cache
class TransformerBlock(nn.Module):
    def __init__(self, dim, num_heads, ffn_expansion_factor=4, bias=False, LayerNorm_type='WithBias', isAtt=True):
        super(TransformerBlock, self).__init__()
        self.isAtt = isAtt
        if self.isAtt:
            self.norm1 = LayerNorm(dim)  # Remove LayerNorm_type argument
            self.attn = Attention(dim, num_heads, bias)
        self.norm2 = LayerNorm(dim)      # Remove LayerNorm_type argument
...
# Downsample and Upsample modules remain unchanged
